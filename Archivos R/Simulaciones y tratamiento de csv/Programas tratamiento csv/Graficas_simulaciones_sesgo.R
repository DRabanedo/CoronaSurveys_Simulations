...
for (i in 1:n_simulaciones) {
    ...
    Nh_basic =      datos_simulacion[,(k+1)+(i-1)*n_estimadores] +  Nh_basic
    Nh_basicvis =   datos_simulacion[,(k+2)+(i-1)*n_estimadores] +  Nh_basicvis
      
    Nh_PIMLE =      datos_simulacion[,(k+3)+(i-1)*n_estimadores] + Nh_PIMLE
    Nh_PIMLEvis =   datos_simulacion[,(k+4)+(i-1)*n_estimadores] + Nh_PIMLEvis
      
    Nh_MLE =        datos_simulacion[,(k+5)+(i-1)*n_estimadores] + Nh_MLE
    Nh_MLEvis =     datos_simulacion[,(k+6)+(i-1)*n_estimadores] + Nh_MLEvis
  
    Nh_MoS =        datos_simulacion[,(k+7)+(i-1)*n_estimadores] + Nh_MoS
    Nh_MoSvis =     datos_simulacion[,(k+8)+(i-1)*n_estimadores] + Nh_MoSvis
      
    Nh_GNSUM =      datos_simulacion[,(k+9)+(i-1)*n_estimadores] + Nh_GNSUM
    
    Nh_Directo =    datos_simulacion[,(k+10)+(i-1)*n_estimadores] + Nh_Directo
}

#¿Que características etaría bien estudiar? ECM, varianza, ...

datos_grafica = data.frame( datos = datos_simulacion$datos, 
               Nh_real     =  Nh_real/n_simulaciones,
...
ggplot(datos_grafica) + 
  ...
  geom_line(aes(x = datos, y =  Nh_real, col = "Nh_real")) +
