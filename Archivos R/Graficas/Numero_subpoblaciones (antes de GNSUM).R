...
poblaciones_dis = c(0,1) #Poblaciones a distinguir. Son disjuntas y el 0 corresponde a no clasificar al individuo en ninguna
v_probabilidades = c(0.5,0.5) #Probabilidad de cada una de las poblaciones anteriores
...
iteraciones = round(seq(from = 1, to = 50, length.out = 10))
...
for (i in 1:length(iteraciones)) {
  ...
  for(j in 0:n_poblaciones){
    ...
    for(v in 1:N) {
      # Visibilidad de la poblaci?n j por v aplicando un factor de visibilidad para las subpoblaciones
      v_1[v] = round(sum(Poblacion[net_sw[[v]][[1]],]$Poblacion == j)*memory_factor_subpob[(n_poblaciones+1)*(i-1)+j+1])
    }
