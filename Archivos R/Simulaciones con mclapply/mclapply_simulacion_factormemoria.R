##############################################################################
# Simulación del factor memoria con mclapply, dejando el resto de cosas fijas
##############################################################################
...
sub_memory_factor = 0  #Factor memoria de las subpoblaciones
...
iteraciones = seq(from = 0, to = 1, length.out = 21)
...
factormemoria.simulaciones <- function(l,N, poblaciones_dis,v_probabilidades,PropHiddenPop, dim, nei, p, visibility_factor,
...
start_time = Sys.time()
lista_simulaciones =mclapply(1:25,factormemoria.simulaciones,N, poblaciones_dis,v_probabilidades,PropHiddenPop, dim, nei, p, visibility_factor,
                             memory_factor,sub_memory_factor,n_encuesta,n_encuesta_hp,iteraciones)
end_time = Sys.time()
...
write.csv(simulaciones,                        # Data frame a ser exportado
          file = "Simulaciones_factormemoria", # Nombre de la hoja de Excel
