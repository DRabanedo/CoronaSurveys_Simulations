...
for (w in 1:length(parameters)) {
  ...
  for (i in 1:(length(v_pop_prob))) {
    for (j in ind1){
      ind2 = dplyr::select(Population, starts_with("subpop") & ends_with(str_c("_", str_c("_", as.character(i) ) )))[,1] != 0
  ...
  for (i in 1:(length(v_pop_prob))) {
    for (j in ind1){
      ind2 = dplyr::select(Population_disjoint, starts_with("subpop") & ends_with(str_c("_", str_c("_", as.character(i) ) )))[,1] != 0
      i_hp_vis[j] = round(rtruncnorm(1, a = -0.5, b =  2*sum(Mhp_vis[ind2,j]) + 0.5, mean = sum(Mhp_vis[ind2,j]), sd = sum(Mhp_vis[ind2,j])*sub_memory_factor)) 
    }
