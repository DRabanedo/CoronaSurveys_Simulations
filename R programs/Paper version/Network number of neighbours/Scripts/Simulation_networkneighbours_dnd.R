...
for (w in 1:length(parameters)) {
  ...
  for (h in 1:N) {
    ...
    vect_hp_vis[h] = round(rtruncnorm(1, a = max(-0.5,  2 * sum(Mhp_vis[h,]) - vect_reach[h] + 0.5 ) , b = min(2 * sum(Mhp_vis[h,]) + 0.5, vect_reach[h]-0.5), mean = sum(Mhp_vis[h,]), sd = mem_factor*sum(Mhp_vis[h,])))
