...
for (h in 1:length(parameters)) {
  list_surveys[[h]] =  gen_Survey(parameters[h], Population)
}

list_surveys_hp = list()
for (h in 1:length(parameters_hp)) {
  list_surveys_hp[[h]] = gen_Survey(parameters_hp[h], Population[Population$hidden_population == 1,])
}
...
for (l in 1:b) {
  ...
  for (i in 1:length(parameters)) {
    ...
    #Surveys variation
    survey = Population[gen_Survey(parameters[i], Population),]
  ...
  for (i in 1:length(parameters)) {
    ...
    #Surveys variation
    survey = Population_disjoint[gen_Survey(parameters[i], Population_disjoint),]
    
    survey_hp_pop = gen_Survey(parameters_hp[i], Population_disjoint[Population_disjoint$hidden_population == 1,])
    survey_hp = Population_disjoint[Population_disjoint$hidden_population == 1,][survey_hp_pop,]
    ...
    vf_estimate_disjoint = VF_Estimate(survey_hp_vf)
