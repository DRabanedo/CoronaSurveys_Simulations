...
for (w in 1:length(parameters)) {
  ...
  for (l in 1:b) {
    ...
    Nh_GNSUM   =  getNh_GNSUM(Population, survey, survey_hp, Mhp_vis, v_pop_total, N, sub_memory_factor)
  ...
  for (l in 1:b) {
    ...
    Nh_GNSUM_disjoint   =  getNh_GNSUM(Population_disjoint, survey, survey_hp, Mhp_vis, v_pop_total_disjoint, N, sub_memory_factor)
...
write.csv(simulaciones,                                             # Data frame
          file = "Simulation_subpopulationnumber_notdisjoint.csv",  # CSV name
...
write.csv(simulaciones_disjoint,                                   # Data frame
          file = "Simulation_subpopulationnumber_disjoint.csv",    # CSV name
          row.names = TRUE )                          # row names: TRUE or FALSE 
