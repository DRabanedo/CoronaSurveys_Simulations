...
vf_dt_ut = function(survey_hp,Population, Mhp_vis,memory_factor){
  ...
  for (i in 1:length(vect_reach_hp)) {
    ...
    mem_vect_reach_hp[i] = max(0,round(rtruncnorm(1, a = -0.5 , b = mem_vect_reach[i], mean = vect_reach_hp[i] , sd = vect_reach_hp[i]*memory_factor)))
  }
...
vf_ut_bin = function(survey_hp,Population, Mhp_vis,memory_factor){
  ...
  # Double trunc + binomial calculate
  for (i in 1:length(vect_reach_hp)) {
    mem_vect_reach[i] = max(0,round(rtruncnorm(1, a = 0.5 + vect_reach_hp[i] , b = (-0.5) + Inf, mean = vect_reach[i], sd = memory_factor*vect_reach[i])))
    mem_vect_reach_hp[i] = rbinom(1, size = mem_vect_reach[i], p = vect_reach_hp[i]/vect_reach[i])
  }
...
vf_ut_ut = function(survey_hp,Population, Mhp_vis,memory_factor){
  ...
  for (i in 1:length(vect_reach_hp)) {
    mem_vect_reach[i] = max(0,round(rtruncnorm(1, a = 0.5 + vect_reach_hp[i] , b =Inf, mean = vect_reach[i], sd = memory_factor*vect_reach[i])))
    mem_vect_reach_hp[i] = max(0,round(rtruncnorm(1, a = -0.5, b = mem_vect_reach[i], mean = vect_reach_hp[i] , sd = vect_reach_hp[i]*memory_factor)))
  }
...
###################################
# Método simple sin mantener nada #
###################################

vf_nm_bin_dt =  function(survey_hp, Population, Mhp_vis,memory_factor){
    # Variables #
    ind_survey = as.numeric(rownames(survey_hp))
    vect_reach_hp = colSums(Mhp_vis[,ind_survey])
    vect_reach = Population$Reach[ind_survey]
    mem_vect_reach_hp = rep(NA,length(vect_reach_hp))
    mem_vect_reach = rep(NA,length(vect_reach_hp))
    mid_value = rep(NA,length(vect_reach_hp))
    ...
    for (i in 1:length(vect_reach_hp)) {
        ...
        mid_value[i] = rbinom(1, size = mem_vect_reach[i], p = vect_reach_hp[i]/vect_reach[i])
        if (mid_value[i] == mem_vect_reach[i]){
          mem_vect_reach_hp[i] = mid_value[i]
        } else {
          mem_vect_reach_hp[i] = max(0,round(rtruncnorm(1, a = 0.5 + 2*mid_value[i] - mem_vect_reach[i], b = (-0.5) + mem_vect_reach[i], mean = mid_value[i] , sd = mid_value[i]*memory_factor)))   
        }
    }
...
visibility_factor = 0.6         # Visibility factor (Binomial's probability)
...
a = array()
for (i in 1:10000){
 a[i] = vf_nm_bin_dt(survey_hp,Population, Mhp_vis, memory_factor)
}
summary(a)

a = array()
for (i in 1:10000){
  a[i] = vf_dt_dt(survey_hp,Population, Mhp_vis, memory_factor) 
}
summary(a)
V